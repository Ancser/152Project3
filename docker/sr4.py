...
# Backoff for retransmissions
retransmission_backoff = {}
...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udpSocket:
    ...
    while baseIndex < len(packets):
        ...
        if received:
            ...
            if sizeAckID in duplicate_acks:
                duplicate_acks[sizeAckID] += 1
                if duplicate_acks[sizeAckID] >= 3:
                    print(f"Fast retransmit for packet [{sizeAckID}]")
                    udpSocket.sendto(
                        int.to_bytes(sizeAckID, SEQ_ID_SIZE, byteorder='big', signed=True) + packets[SeqID],
                        SERVER_ADDRESS
                    )
                    duplicate_acks[sizeAckID] = 0  # Reset after retransmission
        ...
        # Handle timeouts and retransmissions with backoff
        now = time.time()
        for SeqID in range(baseIndex, newIndex):
            sizeSeqID = SeqID * MESSAGE_SIZE
            if sizeSeqID in sentTime and (now - sentTime[sizeSeqID]) > TIMEOUT:
                # Apply backoff
                if sizeSeqID not in retransmission_backoff:
                    retransmission_backoff[sizeSeqID] = 1
                else:
                    retransmission_backoff[sizeSeqID] *= 2  # Exponential backoff

                backoff_timeout = TIMEOUT * retransmission_backoff[sizeSeqID]
                if (now - sentTime[sizeSeqID]) > backoff_timeout:
                    udpPacket = int.to_bytes(sizeSeqID, SEQ_ID_SIZE, byteorder='big', signed=True) + packets[SeqID]
                    udpSocket.sendto(udpPacket, SERVER_ADDRESS)
                    sentTime[sizeSeqID] = now

                    print(f"RE-Sent package [{sizeSeqID}] ({len(packets[SeqID])} bytes) with backoff >>>")
                    totalRetransmission += 1
