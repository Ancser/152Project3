...
with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
    ...
    #         # check if finack message XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
    #         if message == b'==FINACK==':
    ...
    while True:
        ...
        message = packet[SEQ_ID_SIZE:]
        
        # check end signal
        if message == b'==FINACK==':
            print("Received FINACK, stopping receiver.")
            break

        # compare ack and get if right
        if seq_id == EXPECTED_SEQ_ID:
            RECEIVED_DATA[seq_id] = message
            print(f"Received packet {seq_id}")
            EXPECTED_SEQ_ID += 1

        # Return ack
        ack = int.to_bytes(EXPECTED_SEQ_ID, SEQ_ID_SIZE, byteorder='big', signed=True)
        udp_socket.sendto(ack, client)
